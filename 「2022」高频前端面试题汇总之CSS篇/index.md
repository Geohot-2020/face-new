...
- link支持使用**Javascript控制DOM去改变样式**；而@import**不支持**。
...
**总结：** 伪类是通过在**元素选择器**上加⼊伪类改变元素状态，⽽伪元素通过对**元素的操作**进⾏对元素的改变。 
...
- `CSS Sprites`能**减少图片的字节**，把3张图片合并成1张图片的字节总是小于这3张图片的字节总和。
...
- `CSSSprites`在**开发的时候相对来说有点麻烦**，需要借助`photoshop`或其他工具来对每个背景单元**测量其准确的位置**。
